...
final class ExerciseSuggestionTests: XCTestCase {
    @MainActor func testNativeRuntimeDoesNotChangeWorkoutAndPersistsCorrection() async throws {
        ...
        let receiptOrigin = ProcessInfo.processInfo.systemUptime
        for enabled in [false,true] {
            let provider = MockMotionProvider()
            let capture = CaptureModel(provider:provider,recorder:MockRecorder())
            let folder = directory.appendingPathComponent(enabled ? "on" : "off")
            var aiRequests: [AISetRequest] = []
            ...
            capture.startMotion()
            provider.emit(.sample(experimentalRawSample(index:0,time:0,receiptTime:ProcessInfo.processInfo.systemUptime)))
            ...
            for index in 0...510 {
                ...
                await workout.ingest(experimentalRawSample(index:UInt64(index),time:t,
                    ...
                    gravity:.init(x:0,y:0,z:-1),receiptTime:receiptOrigin+t))
            }
            observed.append((workout.session?.current?.reps.map(\.end) ?? [],workout.repSpeedSeries.points.map(\.speed),workout.reps))
            let before=workout.session?.current
            if enabled {
            ...
            for index in 511...545 {
                await workout.ingest(experimentalRawSample(index:UInt64(index),time:Double(index)/50,
                    gravity:.init(x:0,y:0,z:-1),receiptTime:receiptOrigin+Double(index)/50))
            }
            if enabled {
                XCTAssertEqual(workout.exerciseSuggestion.label,.rdl)
                let archive=try JSONDecoder().decode(WorkoutSessionArchive.self,from:Data(contentsOf:XCTUnwrap(workout.sessionURL)))
                XCTAssertEqual(archive.exerciseAnnotations?.first?.confirmedLabel,.rdl)
                ...
                await workout.analyzeLatestSet(model: "stub", apiKey: "")
                XCTAssertEqual(aiRequests.first?.prescription.exercise, .rdl)
                await workout.confirmExerciseSuggestion(.lunge)
                XCTAssertNil(workout.latestSetResult?.aiAdvice)
                await workout.analyzeLatestSet(model: "stub", apiKey: "")
                XCTAssertEqual(aiRequests.last?.prescription.exercise, .lunge)
            }
        }
        XCTAssertEqual(observed[0].0,observed[1].0)
        XCTAssertEqual(observed[0].1,observed[1].1)
        XCTAssertEqual(observed[0].2,observed[1].2)
    }
    func testOldArchiveDecodesWithoutAnnotations() throws {
        var reducer=WorkoutSessionReducer(prescription:WorkoutPrescription())
        reducer.apply(.rep(.init(id:"r",start:0,end:2))); reducer.apply(.closeSet(3))
        let old=WorkoutSessionArchive(schemaVersion:1,sessionID:UUID(),initialPrescription:WorkoutPrescription(),
            policy:.init(),inputs:reducer.inputs,sets:reducer.sets,interrupted:false,recordingDirectory:nil)
        let data=try JSONEncoder().encode(old)
        let decoded=try JSONDecoder().decode(WorkoutSessionArchive.self,from:data)
        XCTAssertNil(decoded.exerciseAnnotations); XCTAssertEqual(decoded.replay().sets,reducer.sets)
    }
    private func prediction(_ label: ExerciseLabel = .rdl, at time: Double, score: Double = 0.99) -> ExercisePrediction {
        var scores = [Double](repeating: (1-score)/9, count: 10)
        scores[ExerciseLabel.allCases.firstIndex(of: label)!] = score
        return .init(captureID: UUID(), epoch: 0, windowStart: time-3.98, windowEnd: time, completedAt: time, scores: scores)
    }
    private func feed(_ p: inout ExerciseLabelPolicy, _ label: ExerciseLabel = .rdl, at t: Double, score: Double = 0.99) {
        p.sample(source: t, now: t); p.accept(prediction(label, at: t, score: score))
    }
    func testFirstLabelAtFiveSecondsWithoutReps() {
        var p = ExerciseLabelPolicy(); p.start(now: 0); p.sample(source: 0, now: 0)
        feed(&p, at: 4); feed(&p, at: 4.5); XCTAssertNil(p.state.label)
        feed(&p, at: 5); XCTAssertEqual(p.state.label, .rdl)
    }
    func testUnknownAtSixSecondsAndLaterRecovery() {
        var p = ExerciseLabelPolicy(); p.start(now: 0); p.sample(source: 0, now: 0)
        for t in stride(from: 4.0, through: 6.0, by: 0.5) { feed(&p, at: t, score: 0.5) }
        XCTAssertEqual(p.state.status, .unknown)
        for t in stride(from: 6.5, through: 9.0, by: 0.5) { feed(&p, at: t) }
        XCTAssertEqual(p.state.label, .rdl)
    }
    func testSwitchRemembersIdentityAcrossUnknown() {
        var p = ExerciseLabelPolicy(); p.start(now: 0)
        for t in [4.0,4.5,5.0] { feed(&p, at: t) }
        feed(&p,.bicepsCurl,at:5.5); XCTAssertEqual(p.state.label,.rdl)
        feed(&p,.bicepsCurl,at:6); XCTAssertNil(p.state.label)
        feed(&p,.bicepsCurl,at:6.5); feed(&p,.bicepsCurl,at:7); XCTAssertNil(p.state.label)
        feed(&p,.bicepsCurl,at:7.5); XCTAssertEqual(p.state.label,.bicepsCurl)
    }
    func testGapAndOutOfOrder() {
        var p = ExerciseLabelPolicy(); p.start(now: 0)
        for t in [4.0,4.5,5.0] { feed(&p, at:t) }
        feed(&p,at:6); XCTAssertNil(p.state.label); XCTAssertEqual(p.vectors.count,1)
        p.accept(prediction(.bicepsCurl,at:5.5)); XCTAssertEqual(p.vectors.count,1)
    }
    func testManualWinsThroughTimeoutResetAndFinish() {
        var p = ExerciseLabelPolicy(); p.start(now: 0); p.confirm(.lunge,now:1)
        feed(&p,at:4); p.tick(now:10); p.discontinuity(now:11)
        XCTAssertEqual(p.state.label,.lunge); XCTAssertTrue(p.state.manual)
        p.finish(); p.accept(prediction(at:12)); XCTAssertEqual(p.state.label,.lunge)
        p.start(now:13); XCTAssertNil(p.state.label); XCTAssertFalse(p.state.manual)
    }
    func testAverageMustAlsoQualifyAndWindowBounded() {
        var p = ExerciseLabelPolicy(); p.start(now: 0)
        for t in [4.0,4.5] { feed(&p,.bicepsCurl,at:t) }
        for t in [5.0,5.5,6.0] { feed(&p,at:t) }
        XCTAssertNil(p.state.label); XCTAssertEqual(p.vectors.count,5)
        feed(&p,at:6.5); feed(&p,at:7); XCTAssertEqual(p.state.label,.rdl)
    }
    func testWatchdogAndOneFailureExpiry() {
        var p = ExerciseLabelPolicy(); p.start(now:0)
        for t in [4.0,4.5,5.0] { feed(&p,at:t) }
        feed(&p,at:5.5,score:0.5); p.sample(source:6,now:6); p.tick(now:6)
        XCTAssertNil(p.state.label)
        p.sample(source:7,now:7); p.tick(now:7); XCTAssertEqual(p.state.reason,"inferenceTimeout")
    }
    func testInvalidModelAndScores() throws {
        var p = ExerciseLabelPolicy(); p.start(now:0)
        var invalid = prediction(at:4); invalid.modelID = "wrong"; p.accept(invalid)
        XCTAssertEqual(p.state.status,.unavailable)
        XCTAssertNil(ExerciseLabelPolicy.qualified([Double](repeating:0.1,count:10)))
        XCTAssertThrowsError(try ExerciseFeatures.extract([]))
        XCTAssertEqual(ExerciseLabel.overheadTricepsExtension.exercise,.overheadTricepsExtension)
        XCTAssertNotEqual(ExerciseLabel.overheadTricepsExtension.exercise,.skullCrusher)
    }
    @MainActor func testDefaultOffAndManualConfirmationDoesNotChangeReducer() async {
        let workout = WorkoutModel()
        XCTAssertFalse(workout.exerciseAutoDetect)
        var reducer = WorkoutSessionReducer(prescription: WorkoutPrescription())
        reducer.apply(.rep(.init(id:"one",start:0,end:2)))
        let before = reducer.current
        var policy = ExerciseLabelPolicy(); policy.start(now:0)
        policy.confirm(.rdl,now:3)
        XCTAssertEqual(before,reducer.current)
        XCTAssertEqual(reducer.inputs.count,1)
    }
}
...
final class WorkoutCaptureRoutingTests: XCTestCase {
    ...
    func testPrimaryWorkoutSupportsEveryExerciseWithGenericCounting() {
        ...
        XCTAssertEqual(V2Exercise.allCases.map(\.rawValue), [
            ...
            "Biceps Curl", "External Rotation", "Skull Crusher", "Overhead Triceps Extension", "Lunge", "Bent Over Rows"
        ])
